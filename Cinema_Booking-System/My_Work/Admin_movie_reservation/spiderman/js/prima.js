...
const Delete=(value)=>{
	console.log(localStorage.getItem("NameSeatsJSON"))
	let seat= JSON.parse(localStorage.getItem("NameSeatsJSON"))//string to obj
	// console.log(seat)
	let allotedSeat=seat[value]
	let reserved= JSON.parse(localStorage.getItem("ReservedSeats"))//string to obj
	
	delete seat[value]
	let empty=[]
	reserved.forEach(x=>{
		if (!allotedSeat.includes(x)) {
			empty.push(x)
			
		}

	})
	localStorage.setItem("ReservedSeats",JSON.stringify(empty))
	console.log(seat)
	localStorage.setItem("NameSeatsJSON",JSON.stringify(seat))
	window.location.reload()
